def resolvePublishParameters(
	...
	Topic = str(topicPrefix) + Topic.replace("PI Integration","")
	...
	if payloadFormat == "value":
		...
		Topic = Topic.replace(measurementName, "")
...
#							currentReading = system.tag.readBlocking(["[monitoring]mqttPublish/publishCount"])[0].value
#							newReading = currentReading + 1
#							system.tag.writeBlocking(["[monitoring]mqttPublish/publishCount"], [newReading])
