...
def saveDevice(devEUI, appEUI, appKey, name, serialNumber, modelID, lat, lon,  description, provider, tagPath, image, user, appID = 0, tenantID = 1 ):
	...
	system.perspective.print(deviceJSON)
	...
	if utils.resultParser.isResultSuccess(validationResults):
		...
		if utils.resultParser.isResultSuccess(deviceCreateResult):
			...
			extraConnections = MPC.customizations.actility.addDeviceToExtraActilityConnections(devEUI, modelID, tagPath)

			domainStatus = MPC.customizations.actility.addToDomain(devEUI, tenantID, tagPath)
			...
			if utils.resultParser.isResultSuccess(tagPathSaveResult):
				...
				PIInstanceCreation = MPC.customizations.PIIntegration.createInstance(fullTagPath, name, modelID)
				...
				if not utils.resultParser.isResultSuccess(extraConnections):
					message += ", but there was an issue adding extra connections in Actility {0}".format(extraConnections)
				if not utils.resultParser.isResultSuccess(domainStatus):
					message += ", but there was an issue associating domain Actility {0}".format(domainStatus)
				if not utils.resultParser.isResultSuccess(PIInstanceCreation):
					piErrorMessage = utils.resultParser.getResultMessage(PIInstanceCreation)
					message += " | PI TEMPLATE ERROR: {0}".format(piErrorMessage)
					system.util.getLogger("SiteSync-DeviceOnboarding").warn("PI template creation failed for {0}: {1}".format(devEUI, piErrorMessage))
...
def validateDevice(device, tagPath):
	system.perspective.print(device)
	isValid = True
